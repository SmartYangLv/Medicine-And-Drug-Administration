...
namespace MedicineAdministration
{
    public partial class PurchaseWarehousing : Form
    {
        ...
        {
            var medicinedrug = new MedicineDrug();
            var medicine=from r in medicinedrug .Medicine 
                         orderby r.NO ascending 
                         select r;
            var currentPageMedicine = medicine.Skip((this.currentpageNo - 1) * 10).Take(10);
            this.dgv_PurchaseTable .DataSource =currentPageMedicine.ToList();
        }
        ...
        private void PurchaseWarehousing_Load(object sender, EventArgs e)
        {
            ...
            var medicine = from r in medicineDrug.Medicine
                           select r;
            var currentPageMedicines = medicine.Take(10);
            this.dgv_PurchaseTable.DataSource =currentPageMedicines.ToList();
